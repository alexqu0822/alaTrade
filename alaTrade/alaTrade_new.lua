...
do	--	MAIN
	...
	do	--	communication func
		...
		function NS.NotifyQueryPrice(id, cache_time)
			if SET.query_online then
				local t = GetServerTime();
				if t - cache_time > PRICE_CREDIBLE_CYCLE then
					...
					if id then
						tinsert(query_queue, { id, cache_time });
					end
				end
			end
		end
	...
	do	--	hook Auction
		...
		local function set_price(buyoutPrice, cut)
			if buyoutPrice then
				...
				if AuctionFrameAuctions.priceType == PRICE_TYPE_UNIT then
					if cut then
						set_buyout_price(buyoutPrice - 1);
						set_start_price(buyoutPrice - 1);
					else
						set_buyout_price(buyoutPrice);
						set_start_price(buyoutPrice);
					end
				elseif AuctionFrameAuctions.priceType == PRICE_TYPE_STACK then
					if cut then
						set_buyout_price(buyoutPrice * count - 1);
						set_start_price(buyoutPrice * count - 1);
					else
						set_buyout_price(buyoutPrice * count);
						set_start_price(buyoutPrice * count);
					end
				end
			end
		end
		...
		do	-- hook Blizzard_AuctionUI
			...
			local function NEW_AUCTION_UPDATE_CALLBACK(_name, _caches, _finished)
				...
				if name and _name == name and _caches[1] then
					...
					if _finished and c1[index_buyoutPriceSingle] then
						set_price(c1[index_buyoutPriceSingle], c1[index_owner] ~= PLAYER_NAME);
					end
				else
					CACHES = nil;
					_selected_index = nil;
			...
			local function AuctionDisplayButton_OnClick(self)
				...
				local index = INDICES[dataIndex]
				if CACHES[index] then
					set_price(CACHES[index][index_buyoutPriceSingle], CACHES[index][index_owner] ~= PLAYER_NAME);
				end
			end
